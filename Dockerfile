...
RUN mkdir --parents /home/${LUSER}/running/desertedscorpion
CMD (sudo crond -p) && cp /home/${LUSER}/private/id_rsa /home/${LUSER}/.ssh/id_rsa && chmod 0600 /home/${LUSER}/.ssh/id_rsa && (gpg --allow-secret-key- --import /home/${LUSER}/private/secret.key || true ) &&  (gpg2 --allow-secret-key- --import /home/${LUSER}/private/secret.key || true ) && pass init ${KEY_ID} && git config --global user.email "emory.merryman@gmail.com" && git config --global user.name "Emory Merryman" && pass git init && pass git remote add origin ${PASS_REPO} && pass git fetch origin master && pass git checkout master && pass git rebase origin/master && git -C /home/${LUSER}/running/desertedscorpion clone git@github.com:desertedscorpion/whitevenus.git && /usr/bin/bash
